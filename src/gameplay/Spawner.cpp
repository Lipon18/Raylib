...
Spawner::Spawner(Player* player, const GameAssets& assets) : m_TargetPlayer(player), m_Assets(assets) {}
...
void Spawner::Spawn(std::vector<std::unique_ptr<Zombie>>& zombies) {
    ...
    zombies.push_back(std::make_unique<Zombie>(spawnPos, m_TargetPlayer, m_Assets));
}